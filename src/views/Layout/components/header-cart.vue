<template>
  <div class="cart">
    ...
    <div class="layer" v-if="$store.getters['cart/validList']">
      <div class="list">
        <div class="item" v-for="item in $store.getters['cart/validList']" :key="item.skuId">
          ...
          <i class="iconfont icon-close-new" @click="delectCart(item)"></i>
        </div>
      </div>
      <div class="foot">
        <div class="total">
...
export default {
  ...
  setup () {
    ...
    const delectCart = good => {
      store.dispatch('cart/asyncDelCart', good).then(() => {
